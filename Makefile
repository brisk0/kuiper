...
release: *.h *.c
	clang $(SRCNAME) -o$(FILENAME) -std=$(STD) -lSDL2 -lm -lz -Wall -O2

$(FILENAME): *.h *.c
	clang $(SRCNAME) -o$(FILENAME) -std=$(STD) -lSDL2 -lm -lz -Wall -O0 -g

run: $(FILENAME)
	./$(FILENAME)

win32:
	i686-w64-mingw32-gcc $(SRCNAME) -o$(FILENAME).exe -std=$(STD) $(MINGWSDLARGS) -lm -lz -Wall -O2 -static
